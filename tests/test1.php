...
$selectquestion = $conn->query("SELECT * FROM questions WHERE scenario_id = $scenario_id ORDER BY question_order ASC LIMIT $start, 1");
$question = $selectquestion->fetch(PDO::FETCH_ASSOC);

if(!$question) {
    header("Location: result.php?stat=ok");
    exit(); 
}
